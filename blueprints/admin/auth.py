...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            return redirect(nextPage) if nextPage else redirect('/admin')
        else:
            return render_template('admin/auth/login.html', form=form, commonError='Неправильний email або пароль')
    ...
    if form.validate_on_submit():
        ...
        if user:
            return render_template('admin/auth/register.html', form=form,
                                   commonError='Користувач з введеним email уже існує')
...
def account():
    ...
    return render_template('admin/auth/account.html', form=form)
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            return render_template('admin/auth/reset-password.html', form=form,
                                   commonError='Користувач з введеним email не існує')
        else:
            sendResetEmail(user)
            return render_template('admin/auth/reset-password.html', form=form,
                                   info='Запит на зміну паролю надісланий на пошту')
    return render_template('/admin/auth/reset-password.html', form=form)
...
def resetToken(token):
    ...
    return render_template('/admin/auth/reset-token.html', form=form)
